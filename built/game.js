...
function stateAfterMove(gameState, move) {
...
function rngRange(rng, low, high) {
    rng[0] ^= rng[0] >> 13;
    rng[0] ^= rng[0] << 17;
    rng[0] ^= rng[0] >> 5;
    return (low + (rng[0] % (high - low)));
}
function rolloutGame(gameState, rng) {
    let buffState = {
...
function evaluatePosition(gameState, numGames, targetWinner) {
    let winCnt = 0;
    let loseCnt = 0;
    let rng = newRng();
